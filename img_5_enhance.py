...
def enhance(decide,cnt):
    src = cv2.imread(out_path(cnt))
    
    if(decide==0):# 白平衡
        output = white_balance(src)

    elif(decide==1):# 灰度世界算法
        output = grey_world(src)

    elif(decide==2):# 直方图均衡化
        output = his_equl_color(src)

    elif(decide==3):# Retinex增强算法 # 单尺度Retinex ssr1
        output = single_scale_retinex(src, 300)
        output[output < 0] = 0
    elif(decide==4):# Retinex增强算法 # 单尺度Retinex ssr2
        output = s_s_r(src)

    elif(decide==5):# Retinex增强算法 # 多尺度Retinex msr1
        output = multi_scale_retinex(src, [15, 80, 250])
        output[output < 0] = 0
    elif(decide==6):# Retinex增强算法 # 多尺度Retinex msr2
        output = m_s_r(src, sigma_list=[15, 80, 250])
    elif(decide==7):# Retinex增强算法 # 多尺度Retinex msrcr
        output = m_s_r_c_r(src, sigma_list=[15, 80, 250])

    elif(decide==8):# 自动白平衡 AWB
        output = automatic_white_balance(src)

    elif(decide==9):# 自动色彩均衡 ACE
        output = automatic_color_equalization(src)
